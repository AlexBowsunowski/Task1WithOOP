...
void Graph::check() {
    ...
    isDicotyledonous = ok;
    if (isDicotyledonous)
        std::cout<< "Graph is dicotyledonous";
    else
...
/**
 * Draw Non Dicotyledonous graph,
 * for this, we need array g(adjacency matrix)
 */
...
/**
 * Draw Dicotyledonous graph,
 * for this, we need a vector part
 * (which shows which part the vertex is in)
 * and array g(adjacency matrix)
 */
void Graph::showDicotyledonous(sf::RenderWindow* window) {
    ...
    // Create Vertex
    for (int i = 0; i < n; ++i) {
        // check-in which part the vertex is located and,
        // based on the part, we find the coordinates
        if (part[i] == 0) {
            x1 = 100;
            y1 = 100 + part1*150;
            part1++;
